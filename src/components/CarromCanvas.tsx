...
function drawFeltSurface(ctx: CanvasRenderingContext2D, theme: BoardTheme, activeSide: number) {
  ...
  // --- Diagonal Corner Pocket Arrows ---
  const arrowOrigins = [
    { x: 130, y: 130, tx: 55, ty: 55 },
    { x: 470, y: 130, tx: 545, ty: 55 },
    { x: 470, y: 470, tx: 545, ty: 545 },
    { x: 130, y: 470, tx: 55, ty: 545 },
  ];

  arrowOrigins.forEach((arrow) => {
    ctx.beginPath();
    ...
    ctx.save();
    ctx.translate(arrow.tx, arrow.ty);
    ctx.rotate(angle);
    ctx.beginPath();
    ctx.moveTo(0, 0);
    ctx.lineTo(-12, -5);
